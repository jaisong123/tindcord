#!/usr/bin/env python3
"""
Test script for using Mistral Chat API with vision capabilities to extract text from dating app screenshots.
"""
...
# Mistral API key
MISTRAL_API_KEY = "ZteyVtTiaZIpbzAgplrUXsGEcnuKM0y2"
...
def process_image_with_mistral_chat(image_path):
    """
    Process an image with Mistral Chat API with vision capabilities.
    ...
    try:
        ...
        # Process with Mistral Chat API
        logging.info("Sending request to Mistral Chat API for extraction")
        
        # First, extract the conversation from the image
        extraction_prompt = """
...
"""
        ...
        extracted_conversation = extraction_response.choices[0].message.content
...
"""
        ...
        logging.info("Sending request to Mistral Chat API for Blake's response")
...
- Minimal punctuation, sparse emoji (mainly "😉")"""},
        ...
        logging.exception(f"Error processing image with Mistral Chat API: {e}")
        return None, None
...
def main():
    ...
    extracted_conversation, blake_response = process_image_with_mistral_chat(image_path)
